...
def main():
    ...
    p2 = sub.add_parser(
        "batch-get-or-create",
        help="Resolve many sequences in order; increments uses_count per occurrence.",
    )
    ...
    p2.add_argument(
        "--col", help="CSV column name containing sequences (required when using --csv)"
    )
    p2.set_defaults(func=cmd_batch_get_or_create)

    p3 = sub.add_parser(
        "stats", help="Show database statistics (totals, lengths, recency, top lists)."
    )
    p3.add_argument(
        "--days",
        default="1,10,30",
    ...
    p3.add_argument("--top", default="10", help="How many rows to show in top lists (default: 10)")
    p3.add_argument("--json", action="store_true", help="Output JSON instead of text")
